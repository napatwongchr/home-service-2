...
const serviceListController = {
  ...
  async getService(req, res) {
    try {
      ...
      const subServiceQueryByServiceId = `
      ...
      if (serviceId) {
        ...
        findService.rows[0].created_at = findService.rows[0].created_at
          .toLocaleString()
          .split(", ")
          .join(" ");
        findService.rows[0].updated_at = findService.rows[0].updated_at
          .toLocaleString()
          .split(", ")
          .join(" ");
        const subServiceById = findSubService.rows.map((subService) => {
          subService.created_at = subService.created_at
            .toLocaleString()
            .split(", ")
            .join(" ");
          subService.updated_at = subService.updated_at
            .toLocaleString()
            .split(", ")
            .join(" ");
          return subService;
      ...
      if (
        ...
        if (sort === "ตามตัวอักษร (Ascending)") {
          serviceQuery = `
                        ...
                        ${groupBy}
                        order by service_name collate "th-x-icu" asc
                        `
